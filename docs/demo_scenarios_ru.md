...
В e2e-тестах пользователи создаются напрямую через модель `UserModel`. На демо можно либо воспользоваться готовыми фикстурами (запуск тестов), либо создать пользователей заранее через админские эндпоинты, если это требуется.
